...
```sh
sudo dnf install https://github.com/sanelabz/icon-creator/releases/latest/download/sane-icons.rpm
```
...
sync service automatically; no additional service setup is required.

To update later, run the same command with `dnf upgrade` instead of
`dnf install`:

```sh
sudo dnf upgrade https://github.com/sanelabz/icon-creator/releases/latest/download/sane-icons.rpm
```

### Building from source

Maintainers can manually run **Build and Release RPM** from the repository's
Actions tab. The resulting `sane-icons-rpm` artifact is intended for testing;
published releases provide the one-command installation URL above.
